...
export function FaceGuessPopover({ style, onGuess, onClose }: FaceGuessPopoverProps) {
  ...
  // game layer (the screen's own `fixed inset-0` wrapper around IncognitoPhoto) or the fixed submit
  // button, so every following tap lands offset by that scroll amount - a dead zone the player
  // could only clear by pinch-zooming the whole page. Snapping scroll back to the origin as the
  // popover unmounts (any close path: guess picked, outside tap, or reveal) realigns the layout and
  // visual viewports and restores tap
