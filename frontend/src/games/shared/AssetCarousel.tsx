...
export function AssetCarousel({ assetIds, alt }: { assetIds: string[]; alt: string }) {
  ...
  const goPrev = () => setIndex((i) => (i - 1 + assetIds.length) % assetIds.length)
  const goNext = () => setIndex((i) => (i + 1) % assetIds.length)
      ...
      {showArrows && (
        <>
          <button
            ...
            className="fixed top-1/2 left-[18px] z-20 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full border border-line-strong bg-surface text-body shadow-card transition-colors hover:bg-hover-tint md:left-10"
          >
          ...
          <button
            ...
            className="fixed top-1/2 right-[18px] z-20 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full border border-line-strong bg-surface text-body shadow-card transition-colors hover:bg-hover-tint md:right-10"
          >
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round">
