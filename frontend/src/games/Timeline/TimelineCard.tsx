...
// dates are, so this is a fixed lookup, never computed from a date span. Wide enough on mobile that
// a full "MMM D, YYYY" date fits the strip on one line without truncating.
...
// The big center card shows the whole photo, letterboxed with a little breathing room - a small
// track card instead fills almost the entire card with its crop (only the date strip is exempt),
// since at that size a letterboxed photo would read as mostly empty card.
