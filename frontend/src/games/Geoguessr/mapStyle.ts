 ...
 *
 * Colors are the hex equivalents of index.css's `--color-*` tokens (MapLibre's style JSON needs
 * literal color strings, not CSS `var()`) - kept in sync by hand, same manual-sync convention
 * already used for api/types.ts vs backend/src/api/schemas.py. Roads/rail/aeroway layers and the
 * sprite-based place-dot icons are dropped entirely (irrelevant clutter at the world-guess zoom
 * levels this map is used at) - only land/water/boundaries/place labels remain.
 */

const MAP_BG = "#f3f5f9" // --color-map-bg
const MAP_WATER = "#ced8ec" // --color-map-water
const MAP_LAND = "#e4e8f0" // --color-map-land
const MAP_LAND_DARK = "#d6dbe4" // building outline, one step darker than --color-map-land
const LINE_STRONG = "#d3d8e0" // --color-line-strong
const INK = "#11161f" // --color-ink
const BODY = "#2d333d" // --color-body
const MUTED = "#5d646f" // --color-muted
const PRIMARY = "#3055b6" // --color-primary
...
export const geoguessrMapStyle: StyleSpecification = {
  version: 8,
