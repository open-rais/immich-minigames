...
class TestGetDailyStatus:
    ...
    def test_lists_only_enabled_modes(self, logged_client, db_session):
        ...
        response = logged_client.get("/api/v1/daily", headers={"X-Owner-Id": str(uuid4())})
    ...
    def test_requires_owner_header(self, logged_client, db_session):
        _enable(db_session)
...
class TestCreateDailyGame:
    ...
    def test_status_reflects_the_created_game(self, logged_client, db_session):
        ...
        status = logged_client.get("/api/v1/daily", headers={"X-Owner-Id": owner}).json()
