"""Roadmap #G, phase F2 - DailyService.get_or_create_challenge. Integration tests against the real
...
class TestSpecShapePerGame:
    def test_more_or_less_person_assets_chain_length(self, daily_service, daily_settings_service):
        daily_settings_service.update_settings(MORE_OR_LESS_TYPE, MODE_PERSON_ASSETS, values={"chain_length": 10})

        challenge = daily_service.get_or_create_challenge(_next_date(), MORE_OR_LESS_TYPE, MODE_PERSON_ASSETS)

        assert len(challenge.spec["chain"]) == 11  # chain_length + 1
        assert "id" in challenge.spec["chain"][0]

    def test_more_or_less_album_assets_chain_length(self, daily_service, daily_settings_service):
        daily_settings_service.update_settings(MORE_OR_LESS_TYPE, MODE_ALBUM_ASSETS, values={"chain_length": 10})

        challenge = daily_service.get_or_create_challenge(_next_date(), MORE_OR_LESS_TYPE, MODE_ALBUM_ASSETS)

        assert len(challenge.spec["chain"]) == 11

    def test_geoguessr_rounds(self, daily_service, daily_settings_service):
        daily_settings_service.update_settings(GEOGUESSR_TYPE, MODE_DISTANCE_BETWEEN_GUESS, values={"total_rounds": 2})

        challenge = daily_service.get_or_create_challenge(_next_date(), GEOGUESSR_TYPE, MODE_DISTANCE_BETWEEN_GUESS)

        assert len(challenge.spec["rounds"]) == 2
        assert "id" in challenge.spec["rounds"][0]["main"]
        assert "extras" in challenge.spec["rounds"][0]

    def test_dateguessr_rounds(self, daily_service, daily_settings_service):
        daily_settings_service.update_settings(DATEGUESSR_TYPE, MODE_DAYS_TO_DATE, values={"total_rounds": 2})

        challenge = daily_service.get_or_create_challenge(_next_date(), DATEGUESSR_TYPE, MODE_DAYS_TO_DATE)

        assert len(challenge.spec["rounds"]) == 2
        assert "date" in challenge.spec["rounds"][0]["main"]

    def test_immichdle_target(self, daily_service):
        challenge = daily_service.get_or_create_challenge(_next_date(), IMMICHDLE_TYPE, MODE_PERSON)

        assert "id" in challenge.spec["target"]
        assert "name" in challenge.spec["target"]

    def test_whos_that_person_rounds_cover_the_configured_total(self, daily_service, daily_settings_service):
        daily_settings_service.update_settings(
            WHOS_THAT_PERSON_TYPE, MODE_NAMED_FACES, values={"total_people": 2, "max_hidden_faces": 2}
        )

        challenge = daily_service.get_or_create_challenge(_next_date(), WHOS_THAT_PERSON_TYPE, MODE_NAMED_FACES)

        assert sum(len(r["faces"]) for r in challenge.spec["rounds"]) >= 2
        assert "asset_id" in challenge.spec["rounds"][0]

    def test_effective_settings_are_snapshotted_onto_the_challenge(self, daily_service, daily_settings_service):
        daily_settings_service.update_settings(GEOGUESSR_TYPE, MODE_DISTANCE_BETWEEN_GUESS, values={"total_rounds": 2})
...
class TestExclusionWindow:
    def test_excludes_the_previous_days_target_within_the_window(self, daily_service, daily_settings_service):
        daily_settings_service.update_settings(IMMICHDLE_TYPE, MODE_PERSON, values={"no_repeat_days": 3})
        day1 = _next_date()
        day2 = day1 + timedelta(days=1)

        first = daily_service.get_or_create_challenge(day1, IMMICHDLE_TYPE, MODE_PERSON)
        second = daily_service.get_or_create_challenge(day2, IMMICHDLE_TYPE, MODE_PERSON)

        assert first.spec["target"]["id"] != second.spec["target"]["id"]

    def test_outside_the_window_repetition_is_allowed(self, daily_service, daily_settings_service):
    ...
    def test_more_or_less_never_applies_cross_day_exclusion(self, daily_service, daily_settings_service):
        ...
        first = daily_service.get_or_create_challenge(day1, MORE_OR_LESS_TYPE, MODE_PERSON_ASSETS)
        second = daily_service.get_or_create_challenge(day2, MORE_OR_LESS_TYPE, MODE_PERSON_ASSETS)
...
class TestFallbackWithoutHistoricalExclusion:
    def test_falls_back_when_the_exclusion_leaves_nothing(
        self, daily_service, daily_settings_service, immich_service, monkeypatch
    ):
        daily_settings_service.update_settings(IMMICHDLE_TYPE, MODE_PERSON, values={"no_repeat_days": 3})
        day0 = _next_date()
        seeded = daily_service.get_or_create_challenge(day0, IMMICHDLE_TYPE, MODE_PERSON)
        ...
        fallen_back = daily_service.get_or_create_challenge(day1, IMMICHDLE_TYPE, MODE_PERSON)
    ...
    def test_raises_not_enough_content_when_even_the_fallback_fails(self, daily_service, immich_service, monkeypatch):
        monkeypatch.setattr(immich_service, "get_persons", lambda **kwargs: [])

        with pytest.raises(Exception, match="not enough named people"):
            daily_service.get_or_create_challenge(_next_date(), IMMICHDLE_TYPE, MODE_PERSON)
...
class TestConcurrentGeneration:
    def test_two_racing_generations_agree_on_one_challenge(self, immich_service):
        ...
        def worker(key: str) -> None:
            ...
            try:
                service = DailyService(session, immich_service)
