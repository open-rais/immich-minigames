...
emitted on them never reach it."""
...
def test_query_string_is_included_in_the_logged_path(logged_client, access_log):
    ...
    assert access_log[-1].path == "/api/v1/persons/search?query=foo"
...
def test_allow_listed_route_without_a_cookie_is_still_logged(client, access_log):
    ...
    assert len(access_log) == 1
    assert access_log[0].status == 401
    assert not hasattr(access_log[0], "auth_fail")
