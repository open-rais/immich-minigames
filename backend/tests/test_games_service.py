...
class TestPersonalRecords:
    ...
    def test_returns_the_highest_score_for_the_user(self, games_service, db_session, auth_service):
        user = _register_user(auth_service)
    ...
    def test_no_games_returns_an_empty_list(self, games_service, auth_service):
        user = _register_user(auth_service)
        records = games_service.get_personal_records(user.id)

        assert records == []

    def test_does_not_leak_a_different_users_records(self, games_service, db_session, auth_service):
        alice = _register_user(auth_service)
        bob = _register_user(auth_service)
        ...
        records = games_service.get_personal_records(bob.id)
