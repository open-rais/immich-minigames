...
class TestCreateGame:
    ...
    def test_requires_owner_header(self, logged_client):
        # Roadmap #H, F3 - owner/X-Owner-Id is untouched (still required) until F4; this confirms
        # the middleware being satisfied (a valid session) doesn't also satisfy this separate,
        # still-required header.
        response = logged_client.post("/api/v1/games", json={"type": "more-or-less", "mode": "personAssets"})
...
class TestGetGame:
    ...
    def test_logged_in_game_is_reachable_with_a_different_owner_header(self, client):
        ...
        # real proof of ownership - a stale/rotated X-Owner-Id shouldn't lock the owner out. Every
        # game is created by a logged-in account now (roadmap #H, F3), so this is the normal case,
        # not a special one - X-Owner-Id genuinely never matters once user_id is set.
        owner = str(uuid4())
        _register(client)
    ...
    def test_logged_in_game_returns_401_once_logged_out(self, client):
