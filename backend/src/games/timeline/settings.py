...
for the contract, games/settings_registry.py for how every game's specs get assembled into one
registry and services/game_settings_service.py for how they're read/written."""
...
# Roadmap #G, decision [G] (docs/TODO/TIMELINE.md) - Timeline is chain-shaped like MoreOrLess (its
# daily build_spec pre-generates a fixed-length chain, so it needs chain_length) but its content is
# still concrete assets that shouldn't repeat across days (so it also needs no_repeat_days, unlike
