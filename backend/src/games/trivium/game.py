...
See docs/TODO/TRIVIUM.md §1/§8 for the design history and why this is (deliberately) a single
generic Game/Round pair rather than one subclass per mode: every round has exactly the same shape
(subject + params + 4 alternatives + which is correct + what media to show), and only *how* a
round is generated varies between question types, not the game loop or the scoring itself.

Scoring is linear-by-time (TRIVIUM.md §3), computed entirely in games/trivium/round.py's
TriviumRound.calculate_score - this module only owns the loop: which round comes next, when the
game is over, and the once-per-game "don't repeat a subject" exclusion (TRIVIUM.md §7).
"""
...
MAX_ROUNDS = 0  # 0 = no limit, see TRIVIUM.md §1's optional cap (mirrors Timeline's MAX_CARDS)
...
class TriviumGame(BaseGame):
    ...
    def _used_subject_ids(self) -> frozenset[UUID]:
        ...
        (TRIVIUM.md §7: a person/asset never repeats as a subject twice in the same game, however
        long it runs)."""
