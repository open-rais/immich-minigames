...
contract (games/daily.py) - see docs/TODO/DECOUPLING.md decision E. Lives in its own module (not
services/games_service.py, where this registry used to live) specifically so
services/daily_service.py can read it too without creating the same import cycle
services/errors.py's docstring already documents for NotEnoughContentError: games_service.py
depends on daily_service.py (to delegate challenge generation), so daily_service.py must never
depend back on games_service.py.
"""
