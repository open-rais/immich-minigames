...
Roadmap #H, F5 - keyed by session-or-IP (_session_or_ip_key below), not a trusted proxy header:
...
limiter = Limiter(key_func=_session_or_ip_key, storage_uri=get_settings().rate_limit_storage_uri)
...
# (_session_or_ip_key above - IP or, on /login, always IP since there's no session yet), so it
...
def enforce_login_email_limit(email: str) -> None:
