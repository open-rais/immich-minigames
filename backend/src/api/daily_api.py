...
normal GameModel row underneath, just one services/game_factory.py's GameFactory.from_row already
knows how to rebuild with its frozen content."""
...
from api.deps import get_daily_games_service, get_scores_service
...
def get_daily_status(
    ...
    daily_games_service: Annotated[DailyGamesService, Depends(get_daily_games_service)],
) -> DailyStatusOut:
    today = date.today()
    statuses = daily_games_service.get_daily_status(user.id, today)
...
def create_daily_game(
    ...
    game = daily_games_service.create_daily_game(game_type=game_type, mode=mode, user_id=user.id)
